...
const Dashboard = () => {
  ...
  useEffect(() => {
    const init = async () => {
      const admissionId = String(sessionStorage.getItem('student_admission_id') || '').trim();
      if (!admissionId) {
        navigate('/login');
        return;
      }
      ...
      try {
        // First try to get the student data from the database
        ...
        if (error || !data) {
          ...
          // If no data, try to get from sessionStorage as fallback
          const firstName = sessionStorage.getItem('student_first_name') || '';
          const lastName = sessionStorage.getItem('student_last_name') || '';
          
          setUserProfile({
            name: `${firstName} ${lastName}`.trim() || 'Student',
            email: admissionId,
            age: 0,
            school: 'MESKAYE HIZUNAN MEDHANE ALEM MONASTERY SCHOOL',
            grade: 'Not Available',
            location: 'Addis Abeba, Ethiopia'
          });
          return;
        ...
        // If we have data from Supabase
        const { first_name, last_name, section, school, age: dbAge, date_of_birth } = data;
        ...
        setUserProfile({
          name: [first_name, last_name].filter(Boolean).join(' ') || 'Student',
          email: admissionId,
          age: age || 0,
          school: school || 'MESKAYE HIZUNAN MEDHANE ALEM MONASTERY SCHOOL',
          grade: section ? `Section ${section}` : 'Not Available',
          location: 'Addis Abeba, Ethiopia'
        });

        // Update session storage with the latest data
        sessionStorage.setItem('student_first_name', first_name || '');
        sessionStorage.setItem('student_last_name', last_name || '');
  ...
  const handleLogout = async () => {
    sessionStorage.removeItem('student_admission_id');
    sessionStorage.removeItem('student_first_name');
